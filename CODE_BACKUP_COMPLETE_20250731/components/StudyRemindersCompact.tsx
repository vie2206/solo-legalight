...
const StudyRemindersCompact: React.FC<StudyRemindersProps> = ({ user }) => {
  ...
  const generateAIReminders = () => {
    const mockStudyPattern: StudyPattern = {
      bestStudyTime: '10:00 AM', averageSessionDuration: 45, strongSubjects: ['Legal Reasoning', 'Current Affairs'],
      weakSubjects: ['Quantitative Techniques', 'English'], consistency: 72, lastWeekSessions: 5
    };
    setStudyPattern(mockStudyPattern);

    const aiReminders: StudyReminder[] = [
      {
        id: '1', type: 'streak', priority: 'high', title: '🔥 Keep Your Streak Alive!',
        message: `You're on a ${user?.study_streak || 15}-day study streak! Study today to maintain your momentum.`,
        action: 'Start 30-min session', dueDate: new Date().toISOString(), progress: 85,
      ...
      {
        id: '2', type: 'weakness', priority: 'high', title: '📊 Focus on Weak Areas',
        message: 'Your Quantitative Techniques score is 12% below your target. Time to strengthen this area!',
        action: 'Practice Quant', subject: 'Quantitative Techniques', progress: 45,
      ...
      {
        id: '3', type: 'schedule', priority: 'medium', title: '⏰ Optimal Study Time',
        message: 'Your peak performance time (10:00 AM) is approaching. Schedule your most challenging topics now.',
        action: 'Schedule session', dueDate: new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString(),
      ...
      {
        id: '4', type: 'revision', priority: 'medium', title: '🔄 Revision Alert',
        message: 'Topics studied 3 days ago need revision to move to long-term memory.',
        action: 'Start revision', subject: 'Legal Reasoning', progress: 75,
      ...
      {
        id: '6', type: 'motivational', priority: 'low', title: '⭐ Daily Motivation',
        message: 'Small daily improvements lead to stunning yearly results. You\'ve got this!',
        action: 'Stay motivated', recommendation: 'Celebrate small wins and maintain consistency',
  ...
  const getIconForType = (type: StudyReminder['type']) => {
    const icons = { streak: FireIcon, weakness: ChartBarIcon, revision: BookOpenIcon, goal: TrophyIcon, schedule: ClockIcon, motivational: SparklesIcon };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-500 to-blue-600 rounded-lg p-6 text-white">
        ...
        <p className="text-purple-100 mb-4">Personalized recommendations based on your study patterns and performance data</p>
        
        {studyPattern && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Consistency', value: `${studyPattern.consistency}%` },
              { label: 'Sessions/Week', value: studyPattern.lastWeekSessions },
              { label: 'Avg Duration', value: `${studyPattern.averageSessionDuration}m` },
              { label: 'Peak Time', value: studyPattern.bestStudyTime }
            ].map(({ label, value }) => (
        ...
        {reminders.length === 0 ? (
          ...
          <div className="space-y-4">
            {reminders.map(reminder => {
              ...
              return (
                <div key={reminder.id} className={cn("border-2 rounded-lg p-4 transition-all hover:shadow-md", getPriorityColor(reminder.priority))}>
                  <div className="flex items-start justify-between">
                        ...
                        {reminder.progress !== undefined && (
                          <div className="mb-2">
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2">
                              <div className={cn("h-2 rounded-full transition-all duration-300", getProgressColor(reminder.progress))} style={{ width: `${reminder.progress}%` }} />
                            </div>
                          </div>
                        )}
                    ...
                    <button onClick={() => dismissReminder(reminder.id)} className="ml-4 p-1 hover:bg-gray-200 rounded-full transition-colors" title="Dismiss">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[
            { icon: '📚', title: 'Study Technique', content: 'Try the Pomodoro Technique: 25 minutes focused study + 5 minute break. Your data shows 18% better retention with structured breaks.', color: 'bg-green-50 text-green-800' },
            { icon: '🎯', title: 'Focus Area', content: 'Prioritize Constitutional Law today. Recent performance data suggests this topic has the highest score improvement potential.', color: 'bg-blue-50 text-blue-800' },
            { icon: '⏰', title: 'Timing Tip', content: 'Your attention span peaks at 47 minutes. Take a 10-minute break before continuing to maintain optimal performance.', color: 'bg-purple-50 text-purple-800' },
            { icon: '🔄', title: 'Memory Boost', content: 'Review yesterday\'s Legal Reasoning notes for 10 minutes before starting new topics. This spaced repetition improves long-term retention.', color: 'bg-yellow-50 text-yellow-800' }
          ].map(({ icon, title, content, color }) => (
            <div key={title} className={cn("rounded-lg p-4", color)}>
              <h4 className="font-medium mb-2">{icon} {title}</h4>
...
export default StudyRemindersCompact;