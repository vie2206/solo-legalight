...
import { vocabularyDatabase, passages, challengesDatabase, gkQuestionsDatabase } from './data/CLATReadingData';
import { 
...
// Enhanced Challenges Screen Component
  const ChallengesScreen = () => {
    ...
    const activeChallenges = challengesDatabase.filter(c => !c.completed);
    const completedChallenges = challengesDatabase.filter(c => c.completed);
    
    const filteredChallenges = (activeTab === 'active' ? activeChallenges : completedChallenges)
      .filter(challenge => filterCategory === 'all' || challenge.category.toLowerCase().includes(filterCategory))
      .sort((a, b) => {
        switch (sortBy) {
          case 'progress':
    ...
    const handleClaimReward = (challengeId: number) => {
      ...
      if (challenge && challenge.completed) {
        ...
        setUserStats(prev => ({
          ...prev,
          xp: prev.xp + challenge.reward + (challenge.bonusReward || 0)
        }));
        ...
        setAchievements(prev => [
          ...
          {
            id: Date.now(),
            title: `${challenge.title} Reward Claimed!`,
            description: `Earned ${challenge.reward + (challenge.bonusReward || 0)} XP`,
            icon: '🎁',
            unlocked: true,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-white to-orange-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Complete challenges to earn rewards and unlock achievements</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-6 bg-gradient-to-r from-yellow-100 to-orange-100 border-b border-orange-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-3xl font-bold text-yellow-600">{activeChallenges.length}</div>
              <div className="text-sm text-yellow-700">Active Challenges</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-600">
                {challengesDatabase.reduce((sum, c) => sum + (c.completed ? c.reward : 0), 0)}
              </div>
              <div className="text-sm text-purple-700">XP Earned</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-600">
                {Math.round((completedChallenges.length / challengesDatabase.length) * 100)}%
              </div>
        ...
        <div className="px-6 py-6">
          <div className="space-y-6">
            {filteredChallenges.map((challenge) => (
              <div key={challenge.id} className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300">
                {/* Challenge Header */}
                      ...
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span className="text-xl font-bold text-yellow-600">
                          ...
                          {challenge.bonusReward && (
                            <span className="text-sm text-green-600">+{challenge.bonusReward}</span>
                          )}
                        </span>
                ...
                <div className="px-6 pb-6">
                  {challenge.completed ? (
                    challenge.claimed ? (
                      <div className="flex items-center justify-center py-3 bg-green-100 text-green-700 rounded-lg border border-green-200">
                      ...
                      <button
                        ...
                        className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-3 px-4 rounded-lg font-semibold hover:shadow-lg transition-all duration-200 transform hover:scale-[1.02] flex items-center justify-center space-x-2"
                      >
                        <Trophy className="w-5 h-5" />
                        <span>Claim {challenge.reward + (challenge.bonusReward || 0)} XP Reward</span>
                      ...
                      onClick={() => {
                        ...
                        switch (challenge.type) {
                          ...
                          case 'mastery':
                            ...
                            break;
                          case 'gk':
                            setCurrentScreen('gk-quiz');
                            break;
                          default:
                            setCurrentScreen('home');
                        }
                      }}
                      className="w-full bg-gradient-to-r from-yellow-500 to-orange-600 text-white py-3 px-4 rounded-lg font-semibold hover:shadow-lg transition-all duration-200 transform hover:scale-[1.02] flex items-center justify-center space-x-2"
                    >
                      <Play className="w-5 h-5" />
...
// Enhanced GK Quiz Screen Component
  const GKQuizScreen = () => {
    ...
    const totalQuestions = Math.min(quizSettings.questionCount, gkQuestionsDatabase.length);
    ...
    const startQuiz = () => {
      ...
      if (quizSettings.mode === 'timed') {
        setQuizTimeLeft(quizSettings.questionCount * quizSettings.timePerQuestion);
    ...
    const handleQuizComplete = () => {
      ...
      // Update user stats
      const correctAnswers = Object.values(gkAnswers).filter(a => a.correct).length;
      const accuracy = Math.round((correctAnswers / totalQuestions) * 100);
      
      setUserStats(prev => ({
        ...prev,
        xp: prev.xp + gkScore,
        comprehensionScore: Math.round((prev.comprehensionScore + accuracy) / 2)
      }));
      ...
      setGKStats(prev => ({
        ...
        accuracy: Math.round(((prev.correct + correctAnswers) / (prev.totalQuestions + totalQuestions)) * 100)
      }));
    };
    ...
    const handleGKAnswer = (selectedOption: number) => {
      ...
      setTimeout(() => {
        if (currentGKQuestion < totalQuestions - 1) {
          ...
          <div className="bg-white shadow-sm border-b border-gray-100 px-6 py-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Customize your quiz settings</p>
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-8">
            <div className="max-w-2xl mx-auto">
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-6">Quiz Mode</h3>
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { id: 'practice', title: 'Practice', desc: 'Untimed learning', icon: '📚' },
                    { id: 'timed', title: 'Timed Quiz', desc: 'Race against time', icon: '⏰' },
                    { id: 'challenge', title: 'Daily Challenge', desc: 'Compete globally', icon: '🏆' }
                  ].map((mode) => (
                    <button
                      key={mode.id}
                      onClick={() => setQuizSettings(prev => ({ ...prev, mode: mode.id }))}
              ...
              <button
                onClick={startQuiz}
                className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white py-4 px-6 rounded-2xl font-bold text-lg shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02]"
              >
                Start {quizSettings.mode === 'timed' ? 'Timed ' : ''}Quiz
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 text-center">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                  ...
                  <div className="bg-green-50 rounded-xl p-4">
                    <div className={`text-3xl font-bold ${getScoreColor(accuracy)}`}>{accuracy}%</div>
                    <div className="text-sm text-green-500">Accuracy</div>
                  </div>
                  <div className="bg-purple-50 rounded-xl p-4">
                    <div className="text-3xl font-bold text-purple-600">{gkScore}</div>
                    <div className="text-sm text-purple-500">Points</div>
                  </div>
                  <div className="bg-orange-50 rounded-xl p-4">
                    <div className="text-3xl font-bold text-orange-600">{grade}</div>
                    <div className="text-sm text-orange-500">Grade</div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button
                    ...
                    className="flex-1 bg-emerald-500 text-white py-3 px-6 rounded-lg font-medium hover:bg-emerald-600 transition-colors"
                  >
                    Take Another Quiz
                  </button>
                  <button
        ...
        <div className="bg-white shadow-sm border-b border-gray-100 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Question {currentGKQuestion + 1} of {totalQuestions}</h2>
                <p className="text-sm text-gray-500">{quizSettings.mode} mode</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
        ...
        <div className="px-6 py-3 bg-gray-50">
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              ...
              style={{ width: `${((currentGKQuestion + 1) / totalQuestions) * 100}%` }}
            ></div>
          </div>
        </div>
          ...
          <div className="px-6 pb-8 pt-6">
            <div className="max-w-2xl mx-auto">
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-4">
                  <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getCategoryColor(currentQuestion.category)}`}>
                    {currentQuestion.category}
                  </span>
                  <div className="flex items-center space-x-2">
                    <span className={`px-2 py-1 rounded text-xs font-medium border ${getDifficultyColor(currentQuestion.difficulty)}`}>
  ...
  const navigateToScreen = (newScreen: string, message?: string) => {
    setIsLoading(true);
  ...
  const renderScreen = () => {
    ...
    switch(currentScreen) {
      case 'home':
        return <div className={fadeInClass}><HomeScreen /></div>;
      case 'flashcards':
        return <div className={fadeInClass}><FlashcardsScreen /></div>;
      case 'passages':
        return <div className={fadeInClass}><PassagesScreen /></div>;
      case 'reader':
        return <div className={fadeInClass}><ReaderScreen /></div>;
      case 'gk-quiz':
        return <div className={fadeInClass}><GKQuizScreen /></div>;
      case 'challenges':
        return <div className={fadeInClass}><ChallengesScreen /></div>;
      default:
        return (
          <div className={`max-w-7xl mx-auto p-4 ${fadeInClass}`}>
            <button
              onClick={onBack}
            ...
            <div className="text-center p-8">
              ...
              <p className="text-sm text-gray-500 mt-4">Enhanced {currentScreen} screen coming soon...</p>
            </div>
          </div>
        );
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 relative">
      ...
      {showWelcomeAnimation && currentScreen === 'home' && (
        <div className="fixed inset-0 bg-gradient-to-br from-blue-600 to-purple-700 z-50 flex items-center justify-center animate-fadeOut">
          <div className="text-center text-white">
            <BookOpen className="w-20 h-20 mx-auto mb-4 animate-bounce" />
            <h1 className="text-4xl font-bold mb-2">Welcome to Reading Mastery</h1>
            <p className="text-xl opacity-90">Let's enhance your CLAT preparation!</p>
          </div>
        </div>
      )}
